...
namespace AI3D
{
    namespace GUI
    {
        ...
        void FeatureKeypoint::Rescale(const float scale_x, const float scale_y) {
            CHECK_OPTION_GT(scale_x, 0);
            CHECK_OPTION_GT(scale_y, 0);
            x *= scale_x;
            y *= scale_y;
            a11 *= scale_x;
            a12 *= scale_y;
        ...
        GroundControlPoint::GroundControlPoint(const float x, const float y)
            : GroundControlPoint(x, y, 0, 0, 0, 1) {}

        GroundControlPoint::GroundControlPoint(const float x_, const float y_,
            const double a11_, const double a12_,
            const double a13_, const int id_)
            : x(x_), y(y_), a11(a11_), a12(a12_), a13(a13_), index(id_) {}
