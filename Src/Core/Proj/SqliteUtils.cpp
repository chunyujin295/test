...
#include "Core/Proj/VariantUtils.h"
...
namespace AI3D
{
    namespace PROJ
    {
        void Sqlite3Closer::operator()(sqlite3* database) const
        {
            sqlite3_close_v2(database);
        }

        void Sqlite3StatementFinalizer::operator()(sqlite3_stmt* statement) const
        {
        ...
        QSet<QString> SqliteUtils::uniqueFields(sqlite3* connection, const QString& tableName, QString& errorMessage)
        {
            QSet<QString> uniqueFieldsResults;
            char* zErrMsg = 0;
            std::vector<std::string> rows;
            const QByteArray tableNameUtf8 = tableName.toUtf8();
            QString sql = sqlite3_mprintf("select sql from sqlite_master "
        ...
        long long SqliteUtils::nextSequenceValue(sqlite3* connection, const QString& tableName, QString errorMessage)
        {
            long long result{ -1 };
            sqlite3_database_unique_ptr dsPtr;
            dsPtr.reset(connection);
            const QString quotedTableName{ SqliteUtils::quotedValue(tableName) };
        ...
        QString SqliteUtils::quotedString(const QString& value)
        {
            if (value.isNull())
        ...
        QString SqliteUtils::quotedIdentifier(const QString& identifier)
        {
        ...
        QString SqliteUtils::quotedValue(const QVariant& value)
        {
        ...
        QStringList SqliteUtils::systemTables()
        {
        ...
        QString sqlite3_mprintf(const char* format, ...)
        {
            va_list ap;
            va_start(ap, format);
