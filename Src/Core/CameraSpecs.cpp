...
namespace AI3D
{
	namespace CORE
	{
		camera_datasheet_s::camera_datasheet_s()
		{

		}

		camera_datasheet_s::camera_datasheet_s(const std::string maker, const std::string model, const double sensor_size) :
			make_(maker), model_(model), sensor_size_(sensor_size)
		{

		}

		bool camera_datasheet_s::operator==(const camera_datasheet_s& rhs) const
		{
			...
			std::string this_maker = make_;
			std::transform(this_maker.begin(), this_maker.end(), this_maker.begin(), ::tolower);
			std::string rhs_maker = rhs.make_;
			std::transform(rhs_maker.begin(), rhs_maker.end(), rhs_maker.begin(), ::tolower);

			if (this_maker.compare(rhs_maker) == 0)
			{
				if (this_model.compare(rhs_model) == 0)
				{
					return true;
				}
			}
		...
		{
			const std::string sfileDatabase = Application::Getinstance().GetCameraDBPath();
			std::ifstream iFilein = File::OpenIfstreamUtf8(sfileDatabase, std::ios::in);
			if (File::IsFileExistent(sfileDatabase) && iFilein)
			{
				std::string line;
				while (iFilein.good())
				{
					...
					if (!line.empty())
					{
						if (line[0] != '#')
						{
							...
							std::vector<std::string> values_Make_Model;
							values = String::StringSplit(line, ";");
							values_Make_Model = String::StringSplit(values[0], " ");
							if (values.size() == 2)
							{
								vec_database.emplace_back(values_Make_Model[0], 
									String::StringReplace(values[0], values_Make_Model[0] + " ", ""), atof(values[1].c_str())
								);
							}
						}
					}
		...
		{
			std::vector<camera_datasheet_s> vec_database;

			
			if (!ParseDatabase(vec_database))
			{
