    ...
    , mPushProjectionToFront( false )
{
  Q_UNUSED( name );
  //setupUi( this );
  QCheckBox* cbxHideDeprecated = new QCheckBox(this);
  QTreeWidget* lstCoordinateSystems = new QTreeWidget(this);
  QTreeWidget* lstRecent = new QTreeWidget(this);;
  QTextEdit* teProjection = new QTextEdit(this);
  QLineEdit* teSelected = new QLineEdit(this);
 ...
 // mSrsDatabaseFileName = DBPATH; //; QgsApplication::srsDbFilePath();
...
QString QgsProjectionSelector::selectedProj4String()
{
 ...
 // QgsDebugMsg( "USER_CRS_START_ID = " + QString::number( USER_CRS_START_ID ) );
  ...
  {
    databaseFileName = DBPATH/*QgsApplication::qgisUserDbFilePath()*/;
...
void QgsProjectionSelector::loadUserCrsList( QSet<QString> *crsFilter )
{
  ...
  QString databaseFileName = DBPATH/* QgsApplication::qgisUserDbFilePath()*/;
