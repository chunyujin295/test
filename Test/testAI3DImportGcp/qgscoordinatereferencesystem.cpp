...
QList<long> QgsCoordinateReferenceSystem::validSrsIds()
{
  ...
  QStringList dbs = QStringList() << qgisUserDatabaseFilePath();// "D:/" /*QgsApplication::srsDatabaseFilePath()*/ << "E:/"/*QgsApplication::qgisUserDatabaseFilePath()*/;
  ...
  for ( const QString &db : constDbs )
  {
    ...
    while ( true )
    {
      ...
      {
        //CHY QgsMessageLog::logMessage( QObject::tr( "SQLite error: %2\nSQL: %1" ).arg( sql, sqlite3_errmsg( database.get() ) ), QObject::tr( "SpatiaLite" ) );
        break;
      }
    }
  }
...
QgsCoordinateReferenceSystem QgsCoordinateReferenceSystem::fromOgcWmsCrs( const QString &ogcCrs )
{
  ...
  std::cout << ogcCrs.toStdString()<<" srsid "<< crs.d->mSrsId << std::endl;;
...
QgsCoordinateReferenceSystem QgsCoordinateReferenceSystem::fromENUDefinition(const QString& definition)
{
    QgsCoordinateReferenceSystem crs;
...
QgsCoordinateReferenceSystem QgsCoordinateReferenceSystem::fromSpecialEpsg(const QString& definition)
{
    QgsCoordinateReferenceSystem crs;
...
{
    std::cout << definition.toStdString() << std::endl;
  ...
  std::cout << match.captured(1).toStdString() << " ++1+ " << match.captured(2).toStdString() << std::endl;
  // 为啥epsg: 4326+5773不会进这里边
  ...
  else
  {
    ...
    std::cout << match.captured(1).toStdString() << " ++ "<< match.captured(2).toStdString() << std::endl;
    if ( match.capturedStart() == 0 )
    {
      ...
      else
      {
          ...
          else if (match.captured(2).startsWith(QLatin1String("enu"), Qt::CaseInsensitive))
          {
    ...
    /*else
    {
        const thread_local QRegularExpression reCrsStr(QStringLiteral("^(epsg|enu)\\:(\\.+)$"), QRegularExpression::CaseInsensitiveOption);
        match = reCrsStr.match(definition);
        if (match.capturedStart() == 0)
        {
            if (match.captured(1).startsWith(QLatin1String("epsg"), Qt::CaseInsensitive))
            {
                result = createFromProj(match.captured(2));
            }
            else
...
bool QgsCoordinateReferenceSystem::createFromOgcWmsCrs( const QString &crs )
{
  ...
  if ( loadFromDatabase(srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/, QStringLiteral( "lower(auth_name||':'||auth_id)" ), wmsCrs.toLower() ) )
  {
    locker.changeMode( QgsReadWriteLocker::Write );
    if ( !sDisableOgcCache )
      sOgcCache()->insert( crs, *this );
  ...
  {
    if ( loadFromDatabase(srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/, QStringLiteral( "lower(auth_name||':'||auth_id)" ), QStringLiteral( "epsg:4326" ) ) )
    {
...
bool QgsCoordinateReferenceSystem::CreateFromSpecialEpsg(const QString& definition)
{


    QgsReadWriteLocker locker(*sCRSSrsIdLock(), QgsReadWriteLocker::Read);// 不知放着行不行
   // auto id = d->mSrsId;
    //if (id >= USER_CRS_START_ID)
    {
      ...
      /*  d.detach();
        ...
        d->mWktPreferred.clear();*/
            ...
            bool result = loadFromDatabase(qgisUserDatabaseFilePath(),
        ...
        //d->mProj4 = "";
        //d->mWktPreferred.clear();
        //d->mDescription = definition;
        //d->mAuthId = definition;// QStringLiteral("%1:%2").arg(auth, code);//这个参数的作用待测
        //auto str = definition.toLower();
        //d->mIsGeographic = str.contains("4326");//这个参数的作用待测
        //d->mAxisInvertedDirty = false;//这个参数的作用待测
        //QString operation;
        //QString ellipsoid;
        ...
        QgsProjUtils::proj_pj_unique_ptr crs(proj_create(pjContext, definition.toLatin1().constData()));
...
bool QgsCoordinateReferenceSystem::CreateFromENUDefinition(const QString& definition)
{
   ...
   // if (id >= USER_CRS_START_ID)
    ...
    //d->mDescription = definition;
    //d->mAuthId = definition;// QStringLiteral("%1:%2").arg(auth, code);//这个参数的作用待测
    //d->mIsGeographic = false;//这个参数的作用待测
    //d->mAxisInvertedDirty = false;//这个参数的作用待测
    //QString operation;
    //QString ellipsoid;
    ////  getOperationAndEllipsoidFromProjString(proj4, operation, ellipsoid);
...
bool QgsCoordinateReferenceSystem::createFromPostgisSrid( const long id )
{
  ...
  bool result = loadFromDatabase(srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/, QStringLiteral( "srid" ), QString::number( id ) );
...
bool QgsCoordinateReferenceSystem::createFromSrsId( const long id )
{
  ...
  bool result = loadFromDatabase( id < USER_CRS_START_ID ? srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/
      :
      qgisUserDatabaseFilePath()/*QgsApplication::qgisUserDatabaseFilePath()*/,
...
bool QgsCoordinateReferenceSystem::loadFromDatabase( const QString &db, const QString &expression, const QString &value )
{
  ...
  */
  ...
  std::cout << mySql.toStdString() << " ++++++++++++" << std::endl;
  ...
  if ( myResult == SQLITE_OK && statement.step() == SQLITE_ROW )
  {
    ...
    std::cout << d->mAuthId.toStdString() << "===================" << __FUNCTION__ << " " << __LINE__ << std::endl;
...
bool QgsCoordinateReferenceSystem::createFromProj( const QString &projString, const bool identify )
{
  ...
  if ( identify )
  {
   ...
   // const QString projCrsString = myProj4String + ( myProj4String.contains( QStringLiteral( "+type=crs" ) ) ? QString() : QStringLiteral( " +type=crs" ) );
    QString projCrsString;
    std::string str = myProj4String.toStdString();
    std::string ret(str);
    if ((str.find("proj=") != std::string::npos ||
        str.find("+proj=") != std::string::npos ||
        str.find("+init=") != std::string::npos ||
        str.find("+title=") != std::string::npos))
    {

        projCrsString = myProj4String + (myProj4String.contains(QStringLiteral("+type=crs")) ? QString() : QStringLiteral(" +type=crs"));
    }
    else
    {
        projCrsString = myProj4String;
    }
    std::cout << projCrsString.toStdString() << " "<< __FUNCTION__<< " "<< __LINE__ << std::endl;
    ...
    if ( crs )
    {
      ...
      //else
      //{
      //    if (CreateFromSpecialEpsg(projCrsString))
      //    {
      //        locker.changeMode(QgsReadWriteLocker::Write);
      //        std::cout << sDisableStringCache << std::endl;
      //        if (!sDisableStringCache)
      //            sStringCache()->insert(projString, *this);
      //        return d->mIsValid;
      //    }
      //    //
      //}
    ...
    if ( !myRecord.empty() )
    {
      ...
      if ( id >= USER_CRS_START_ID )
      {
          myProj4String = myProj4String.toUpper();
          std::cout << myProj4String.toStdString() << std::endl;
         /* if (myProj4String.contains("ENU")|| myProj4String.contains("EPSG"))
          {
              std::cout << __FUNCTION__ << "--------" << __LINE__ << std::endl;
              createFromString(myProj4String);
          }
...
QgsCoordinateReferenceSystem::RecordMap QgsCoordinateReferenceSystem::getRecord( const QString &sql )
{
  ...
  myDatabaseFileName = srsDatabaseFilePath()/* QgsApplication::srsDatabaseFilePath()*/;
  ...
  if ( myMap.empty() )
  {
    myDatabaseFileName = qgisUserDatabaseFilePath()/*QgsApplication::qgisUserDatabaseFilePath()*/;
...
QString QgsCoordinateReferenceSystem::userFriendlyIdentifier( IdentifierType type ) const
{
  if ( !authid().isEmpty() )
  {
      std::cout << description().toStdString() << " == " << authid().toStdString() << " =1== " << description().toStdString() << std::endl;
    ...
    if ( PJ *obj = d->threadLocalProjObject() )
    {
      ...
      if ( ellipsoid )
      {
        ...
        else
        {
          ...
          {
              QString str = QString::number(semiMajor, 'f', 2);
              QString str1 = QString::number(semiMinor, 'f', 2);
            d->mEllipsoidAcronym = QStringLiteral( "PARAMETER:%1:%2" ).arg( str/*qgsDoubleToString( semiMajor )*/,
                                 str1  /*qgsDoubleToString( semiMinor )*/ );
          }
          else
          {
            d->mEllipsoidAcronym.clear();
...
void QgsCoordinateReferenceSystem::setProjString( const QString &proj4String )
{
  ...
  QString projCrsString;
  std::string str = proj4String.toStdString();
  if ((str.find("proj=") != std::string::npos ||
      str.find("+proj=") != std::string::npos ||
      str.find("+init=") != std::string::npos ||
      str.find("+title=") != std::string::npos))
...
long QgsCoordinateReferenceSystem::findMatchingProj()
{
  ...
  // Get the full path name to the sqlite3 spatial reference database.
  QString myDatabaseFileName = srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/;
...
long QgsCoordinateReferenceSystem::saveAsUserCrs( const QString &name, Format nativeFormat )
{
  ...
  sqlite3_statement_unique_ptr statement;
  //check the db is available
  int myResult = database.open(qgisUserDatabaseFilePath()/*QgsApplication::qgisUserDatabaseFilePath()*/ );
  if ( myResult != SQLITE_OK )
  {
    //QgsDebugMsg( QStringLiteral( "Can't open or create database %1: %2" )
    //             .arg("D:/"/*QgsApplication::qgisUserDatabaseFilePath()*/,
  ...
  {
    //CHYQgsMessageLog::logMessage( QObject::tr( "Saved user CRS [%1]" ).arg( toProj() ), QObject::tr( "CRS" ) );
...
long QgsCoordinateReferenceSystem::getRecordCount()
{
  ...
  //check the db is available
  myResult = database.open_v2(qgisUserDatabaseFilePath()/*QgsApplication::qgisUserDatabaseFilePath()*/, SQLITE_OPEN_READONLY, nullptr );
  if ( myResult != SQLITE_OK )
  {
...
QList<long> QgsCoordinateReferenceSystem::userSrsIds()
{
  ...
  const QString db = qgisUserDatabaseFilePath() /*QgsApplication::qgisUserDatabaseFilePath()*/;
...
{
  setlocale( LC_ALL, "C" );
  QString dbFilePath = srsDatabaseFilePath()/*QgsApplication::srsDatabaseFilePath()*/;
  ...
  /*QgsDebugMsgLevel*/( QStringLiteral( "Load srs db from: %1" ).arg("D:/"/*QgsApplication::srsDatabaseFilePath().toLocal8Bit().constData()*/ ), 4 );

  sqlite3_database_unique_ptr database;
  if ( database.open( dbFilePath ) != SQLITE_OK )
  {
    //QgsDebugMsg( QStringLiteral( "Could not open database: %1 (%2)\n" ).arg("D:/"/*QgsApplication::srsDatabaseFilePath()*/, database.errorMessage() ) );
    return -1;
  }
...
// fix up database, if not done already //
  ...
  if ( sqlite3_exec( database.get(), "COMMIT", nullptr, nullptr, nullptr ) != SQLITE_OK )
  {
    ...
    //  "D:/"/*QgsApplication::srsDatabaseFilePath()*/,
    //               sqlite3_errmsg( database.get() ) )
    //           );
    return -1;
  }
...
void QgsCoordinateReferenceSystem::pushRecentCoordinateReferenceSystem( const QgsCoordinateReferenceSystem &crs )
{
  ...
  for ( const QgsCoordinateReferenceSystem &c : ( recent ) )
  {
    authids << c.authid();
    proj << c.toProj();
