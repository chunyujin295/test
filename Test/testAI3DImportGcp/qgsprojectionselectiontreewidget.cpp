  ...
  : QWidget( parent )
{
  ...
  mSrsDatabaseFileName = srsDatabaseFilePath();// QgsApplication::srsDatabaseFilePath();
...
void QgsProjectionSelectionTreeWidget::showEvent( QShowEvent *event )
{
  ...
  {
    for ( const QgsCoordinateReferenceSystem &crs : ( mRecentProjections ) )
      insertRecent( crs );
...
QString QgsProjectionSelectionTreeWidget::getSelectedExpression( const QString &expression ) const
{
  ...
  long crsid = lvi->text(QgisCrsIdColumn).toLong();
  if ( lvi->text( QgisCrsIdColumn ).toLong() >= USER_CRS_START_ID )//USER_CRS_START_ID
  {
      databaseFileName = qgisUserDatabaseFilePath();// QgsApplication::qgisUserDatabaseFilePath();
...
QgsCoordinateReferenceSystem QgsProjectionSelectionTreeWidget::crs() const
{
  ...
  if (1)
  {
       QString authidString = getSelectedExpression(QStringLiteral("auth_id")); 
      const QString srsIdString = getSelectedExpression(QStringLiteral("srs_id"));
      std::string srsidstr = srsIdString.toStdString();
      std::cout << authidString.toStdString() <<"+++ " << srsidstr << " " << __FUNCTION__ << " " << __LINE__ << std::endl;;
      if (!srsIdString.isEmpty())
      {
          ...
          {
              authidString =  authidString.toUpper();
              if (authidString.contains("ENU:"))
              {
                  return QgsCoordinateReferenceSystem::fromENUDefinition(authidString);
              }
              else if (authidString.contains("EPSG:"))
              {
                  return QgsCoordinateReferenceSystem::fromSpecialEpsg(authidString);
              }
              else 
              {
...
void QgsProjectionSelectionTreeWidget::loadUserCrsList( QSet<QString> *crsFilter )
{
  ...
  QString databaseFileName = qgisUserDatabaseFilePath();// QgsApplication::qgisUserDatabaseFilePath();
  ...
  if ( result == SQLITE_OK )
  {
    ...
    while ( sqlite3_step( stmt ) == SQLITE_ROW )
    {
      ...
      std::string colname = QString::fromUtf8((char*)sqlite3_column_text(stmt, 1)).toStdString();
      int  coln = QString::fromUtf8((char*)sqlite3_column_text(stmt, 1)).toInt();
